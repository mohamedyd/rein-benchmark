####################################################
# Benchmark: script for testing the robustness of the error detection methods
...
if __name__ == '__main__':
    ...
    parser.add_argument('--run_error_rate', action='store_true')
    parser.add_argument('--run_outlier_degree', action='store_true')
    ...
    # Define the experiment to run, i.e., error_rate or outlier_detection
    error_rate_bool = False if args.run_outlier_degree else True

    # All detectors are executed, iff empty list is passed to which_detectors
    app.run_robustness_exp(datasets_list, detectors_list, iterations, error_rate_bool)
