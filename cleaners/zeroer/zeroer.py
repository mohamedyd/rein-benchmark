...
def get_zeroer(dataset_name, dataset_path, run_trans = False,LR_dup_free = False):
    """"function that can be called from other projects to run zeroer"""
    
    LR_dup_free = LR_dup_free
    run_trans = run_trans
    dataset_name = dataset_name
    dataset_path = dataset_path
    
    # if blocking function is defined for dataset use it, otherwise
    # use generic/default one
    if dataset_name in blocking_functions_mapping:
        blocking_func = blocking_functions_mapping[dataset_name]
    else:
        blocking_func = blocking_functions_mapping['default']
