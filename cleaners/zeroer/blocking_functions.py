...
def blocking_for_citeseer_dblp(A,B):
    ...
    C1 = ob.block_tables(A, B, 'title', 'title', word_level=True, overlap_size=10, show_progress=True,
...
def generic_blocking_func(A, B,increase_o=0):
...
def movie_blocking_func(A,B,increase_o=0):
    ob = em.OverlapBlocker()
    C = ob.block_tables(A, B, "title", "title", overlap_size=3, show_progress=True, allow_missing=False,n_jobs=-1)
    return C

def airbnb_blocking_func(A,B,increase_o=0):
    #ob = em.OverlapBlocker()
    #C = ob.block_tables(A, B, "LocationName", "LocationName", overlap_size=1, show_progress=True, allow_missing=False,n_jobs=-1)
    ab = em.AttrEquivalenceBlocker()
    C = ab.block_tables(A,B,"LocationName", "LocationName", n_jobs=-1)
    return C

def citation_blocking_func(A,B,increase_o=0):
    ob = em.OverlapBlocker()
    C = ob.block_tables(A, B, "title", "title", overlap_size=8, n_jobs=-1)
    
    return C

def test(A,B,increase_o=0):
    ob = em.OverlapBlocker()
    C = ob.block_tables(A, B, 'first', 'first', l_output_attrs=['first', 'second'],  r_output_attrs=['first', 'second'],
        overlap_size=1+increase_o, show_progress=False)
    return C

def movie_blocking_func(A,B,increase_o=0):
    ob = em.OverlapBlocker()
    C = ob.block_tables(A, B, "city", "city", overlap_size=1, show_progress=True, allow_missing=False,n_jobs=-1)
    return C

blocking_functions_mapping = defaultdict(str)
blocking_functions_mapping["default"] = generic_blocking_func
blocking_functions_mapping["airbnb"] = airbnb_blocking_func
blocking_functions_mapping["citation"] = citation_blocking_func

blocking_functions_mapping["test"] = movie_blocking_func
