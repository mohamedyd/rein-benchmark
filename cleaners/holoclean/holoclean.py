...
class Session:
    ...
    def detect_errors(self, detect_list, return_errors=False):
        status, detect_time, errors_df = self.detect_engine.detect_errors(detect_list)
        logging.info(status)
    ...
    def repair_errors(self, featurizers):
        ...
        status, time, repaired_df = self.ds.get_repaired_dataset()
        logging.info(status)
