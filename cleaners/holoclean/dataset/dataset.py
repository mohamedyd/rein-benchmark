...
class Dataset:
    ...
    def load_data(self, name, fpath, na_values=None, entity_col=None, src_col=None, df=None):
        """
        ...
        try:
            ...
            # Load raw CSV file/data or dataframe into a Postgres table 'name' (param).
            if not df.empty:
                self.raw_data = Table(name, Source.DF, na_values=na_values, exclude_attr_cols=exclude_attr_cols, df=df)
            else:
    ...
    def get_repaired_dataset(self):
        ...
        print("REPAIRED DF: ", repaired_df)
        #repaired_df.to_csv("/home/chrha/Desktop/holoclean_examples/repaired_df_from_benchmark.csv", index=False, encoding="utf-8")
        ...
        toc = time.time()
        total_time = toc - tic
        return status, total_time, repaired_df
