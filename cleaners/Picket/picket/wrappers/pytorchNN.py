...
class Net(nn.Module):
	def __init__(self, input_size, layers_size=100, output_size=2, num_layers=3, dropout=None):
...
class torchNN:
	def __init__(self, input_size, layers_size=100, output_size=2, num_layers=3, dropout=None, weight_decay=0, batch_size=100):
	...
	def fit(self, X, y):
		...
		self.classifier.fit(X, toOneHot(y), batch_size=self.batch_size, nb_epochs=100)

	def predict(self, X):
		y_pred = np.argmax(self.classifier.predict(X, batch_size=self.batch_size), axis=1)
		return y_pred
